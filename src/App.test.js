...
//   // expect(queryByText(/fetching data/i)).not.toBeNull();
// });

test("dropdown renders", () => {
  const { queryByTestId, queryByText } = render(<Dropdown />);
  queryByTestId(/react-dropdown/i);
  expect(queryByText(/fetching data/i)).toBeNull();
});

jest.mock("./api/fetchShow");

test("App renders the show data", async () => {
...
test("Dropdown renders", async () => {
  ...
  wait(() => {
    // userEvent(seasonOne);
    expect(getByText(/season 1/i)).toHaveLength(8);

    const seasonOne = userEvent.click(getByText(/season 1/i));
    const seasonTwo = userEvent.click(getByText(/season 2/i));
    const seasonThree = userEvent.click(getByText(/season 3/i));
    const seasonFour = userEvent.click(getByText(/season 4/i));
    expect(seasonOne).toHaveLength(1);
    expect(seasonTwo).toHaveLength(1);
    expect(seasonThree).toHaveLength(1);
    expect(seasonFour).toHaveLength(2);
  });
});

// test("Each selection renders", async () => {
//   const { getByText } = render(<App />);
